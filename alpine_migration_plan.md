...
- **Remove all `render*()` functions** → Alpine auto-updates
- **Remove all `onclick` attributes** → Use `@click` directives
- **Remove `window.*` exports** → Alpine scopes data properly
- **~40% code reduction** → Less to maintain
...
**Key difference:** No manual `renderCharacters()` call needed!
...
export function renderCharacters() {
    ...
    group.characters.forEach(char => {
        ...
        card.innerHTML = `
            <button onclick="removeChar(${char.id})">❌</button>
...
<div x-data="$store.game">
    <template x-for="char in currentGroup.characters" :key="char.id">
        <div class="hero-card">
            <button @click="removeChar(char.id)">❌</button>
...
1. **js/modules/char.js** - Remove `renderCharacters()`, keep business logic
2. **js/modules/group.js** - Remove `renderGroupSelector()`, keep logic
3. **js/modules/inventory.js** - Remove `renderInventory()`, keep logic
...
### **Potential Concerns:**
1. **Character list rendering:** ~10 characters × complex template
...
<section id="gruppe" x-data="$store.game" class="game-section">
    ...
    <button @click="renameGroup(prompt('Neuer Name?'))">✏️</button>
    <button @click="deleteGroup()">🗑️</button>
...
1. **Quick rollback:** Restore `index.backup.html` + old JS
...
- **Setup & Planning:** 30 min ✅ (this document)
