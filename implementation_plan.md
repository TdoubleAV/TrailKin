...
## Ziel
Aufteilung der monolithischen `index.html` in wartbare, isolierte JavaScript-Module. Dies verhindert, dass ein Fehler die gesamte App (insbesondere die Navigation) lahmlegt.
...
- [ ] **State:** `js/state.js` erstellen. Beinhaltet `appState`, `loadGame`, `saveGame` und Hilfsfunktionen. Exportiert das State-Objekt und Methoden.
...
- [ ] **Modal:** `js/modules/modal.js` erstellen (exportiert `showInputModal`).

### 4. Feature Module (Logic & Render)
Jedes Modul importiert den State und notwendige Helper.
- [ ] **Group:** `js/modules/group.js` (Logik + `renderGroupSelector`).
- [ ] **Char:** `js/modules/char.js` (Logik + `renderCharacters`).
- [ ] **Inventory:** `js/modules/inventory.js` (Logik + `renderInventory`).
- [ ] **Quest:** `js/modules/quest.js` (Logik + `renderQuest`).

### 5. Integration (`main.js` & `index.html`)
- [ ] **Main:** `js/main.js` importiert alle Module. Setzt globale Event-Listener (falls nötig für HTML `onclick`) oder – besser – registriert Events per JS (`addEventListener`).
- [ ] **HTML:** Entfernen des gesamten `<script>` Blocks aus `index.html`. Einfügen von `<script type="module" src="js/main.js"></script>`.

## Wichtig: Globale Funktionen
Aktuell nutzt `index.html` viele `onclick="someFunction()"`. Da Module ihren Scope isolieren, sind diese Funktionen nicht mehr global verfügbar (`window.someFunction`).
**Lösung:**
...
```
*Langfristig wäre es sauberer, `onclick` im HTML zu entfernen und `addEventListener` zu nutzen, aber für diesen Schritt pragmatischer.*
